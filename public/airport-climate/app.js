...
function select(id,scroll=false){
 ...
 <div><b>Climate constraint</b>${esc(r.prominence)}</div><div><b>National goal</b>${esc(r.national_climate_goal)}</div><div><b>Aviation-sector goal</b>${esc(r.aviation_climate_goal)}</div><div><b>International aviation</b>${esc(r.international_aviation_included)}</div>
 </div><p class="detail-note"><strong>How climate enters the decision:</strong> ${esc(r.evidence_note)}</p>
 <p class="sources">${sourceLink(r.project_source,'Project source')}${r.project_source&&r.climate_source?' · ':''}${sourceLink(r.climate_source,'Climate / policy source')} · Research confidence: ${esc(r.confidence)}</p>${r.verification_note?`<p class="sources">Checked 16 September 2026: ${esc(r.verification_note)} ${sourceLink(r.verification_source,'Verification source')}</p>`:''}`;
